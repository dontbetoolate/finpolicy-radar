...
- 国家金融监督管理总局使用公开文档 API 发现候选并读取正文，同时保留可访问的官方详情页 URL。
...
- `nfra_api` 调用总局公开文档列表 API，使用真实 `docId` 构造官方详情页 URL，并拒绝缺失或含模板表达式的字段。
...
总局候选不解析 Angular 页面模板，而是使用同一真实 `docId` 调用公开详情 API，将 `docClob` 清理为正文文本；对外 URL 仍指向总局官方详情页。中国政府网候选继续使用通用详情页解析。
