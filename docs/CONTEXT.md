...
- 中国人民银行和国家网信办使用通用 HTML 解析发现链接并抓取详情页。
- 国家金融监督管理总局使用公开文档 API 发现候选并读取正文；主域名接口失败时依次尝试其官方 Big5 镜像，同时保留简体中文官方详情页 URL。
- 中国政府网使用国务院政策文件库公开检索 API 发现候选，再抓取真实官方详情页。
- 规范化 URL，去除部分跟踪参数，并按候选键和 URL 合并去重。
- 从详情页 `<h1>` 或列表标题提取标题，从页面文本或 URL 提取日期，并按正文选择器优先级、链接密度和导航噪声选择正文。
...
| `config/sources.yaml` | 官方来源、采集器类型、入口或公开 API、允许域名、URL 规则及来源权重 |
...
- 未配置 `collector` 的来源处理其 `start_urls`：
...
- `nfra_api` 依次调用总局主域名和官方 Big5 镜像的公开文档列表 API，使用真实 `docId` 构造官方详情页 URL，并拒绝缺失或含模板表达式的字段。
- `gov_policy_api` 按配置关键词调用国务院政策文件库公开检索 API，去除标题高亮标签，仅保留标题、接口摘要或发布机构含金融语境词的候选，再校验官方域名和路径、按 URL 去重并按发布日期排序。
...
总局候选不解析 Angular 页面模板，而是使用同一真实 `docId` 调用公开详情 API，将 `docClob` 清理为正文文本；主接口失败时切换至官方 Big5 镜像，对外 URL 仍指向简体中文官方详情页。中国政府网候选继续使用通用详情页解析。

### 5.4 有效性与相关性过滤

- 候选 URL 含未解析模板表达式时跳过。
- 详情标题、URL 或正文仍含模板表达式时跳过。
- 正文过短、包含大量网站导航文案、标题属于会议会见等新闻动态，或内容属于单项授权等一次性行政信息时跳过。
- 标题必须包含办法、规定、通知、意见、指引、规划、公告、政策解读等政策材料信号；仅在新闻或署名文章正文中提及这些词不算政策材料。
...
- 重要性：在通过政策材料资格判断后，由相关性、文件类型、标题信号和来源权重共同决定 1—5 星；监管动态最高 2 星、政策解读最高 3 星，五星保留给高相关的正式制度文件。
- 摘要：先移除重复标题、发布日期、来源和打印纠错控件，再从正文中选择最多两句、约 150 字以上的有效句子，最终截断为 220 字；无可用句子时使用固定提示。
...
- 合并排序后，同一来源内标题完全相同的记录只保留最新发现的一条；不同来源发布同名文件时仍分别保留，以维持官方来源可追溯性。
...
| `source_id` | 来源 | 采集方式 |
| --- | --- | --- |
| `pbc_news` | 中国人民银行 | 通用 HTML |
| `nfra_policy` | 国家金融监督管理总局 | 公开文档 API |
| `gov_policy` | 中国政府网 | 政策文件库公开检索 API + HTML 详情 |
| `cac_policy` | 国家互联网信息办公室 | 通用 HTML |
...
- 政府网页或公开接口变化、反爬措施、网络波动，以及官方站点对 GitHub Actions 运行器的访问限制，可能导致来源失败或零候选。这是持续运行风险，不是已经解决的问题；当前通过 `status.json` 和来源失败隔离降低影响，后续仍需增加监控。
...
5. 增加正文质量、发布日期、官方 URL 和专用 API 字段的来源级回归样本。
6. 设计同 URL 政策修订与失效检测。
7. 在上述基础稳定后，再评估新增来源、通知集成或可选大模型分析。
