...
- 国家金融监督管理总局分页读取现行规章、行政规范性文件和其他文件三个公开栏目，在标题阶段聚焦金融科技候选，再通过公开文档 API 读取正文；主域名接口失败时依次尝试其官方 Big5 镜像，同时保留简体中文官方详情页 URL。
...
- 使用关键词相关性分数过滤政策。
- 使用规则生成兼容分类、适用对象、文件类型、三级关注级别、摘要和影响提示。
- 将政策与采集状态保存在 JSON 文件中。
- 使用 Jinja2 生成静态首页和 RSS；浏览器端 JavaScript 提供搜索、来源、适用对象、文件类型、日期范围和关注级别筛选。
...
| `config/rules.yaml` | 抓取限制、相关性关键词、兼容分类、适用对象、文件类型和关注级别规则 |
...
| `public/assets/app.js` | 浏览器端搜索、来源、适用对象、文件类型、日期范围和关注级别筛选 |
...
- 分类：任一分类关键词命中即可进入相应分类；该字段为历史兼容数据，当前首页不展示或筛选。
- 适用对象：优先根据标题，其次根据正文开头明确指向判断；允许多个对象，无法可靠判断时使用“未明确”。
- 文件类型：优先依据官方栏目和标题发布形式，按解读、征求意见、规划方案、法规规章、标准规范、规范性政策文件的顺序判断，不使用正文中的偶然引用词。
- 关注级别：保存为兼容整数 5、3、1，对应核心关注、重点跟踪和一般参考；政策解读固定为一般参考，核心关注要求标题直接涉及核心领域且影响多个主体或整个行业。
...
| `document_type` | 字符串 | 规则识别的发布形式或“其他政策材料” |
| `applicable_entities` | 字符串列表 | 文件明确适用的一个或多个对象；无法可靠判断时为“未明确” |
| `categories` | 字符串列表 | 一个或多个规则分类 |
| `keywords` | 字符串列表 | 最多保存前 10 个相关性命中词 |
| `relevance_score` | 整数 | 关键词权重之和 |
| `importance` | 1、3、5 整数 | 三级关注级别的兼容存储值：一般参考、重点跟踪、核心关注 |
...
| `nfra_policy` | 国家金融监督管理总局 | 现行三个政策栏目分页列表 API + 公开详情 API |
...
- 关键词规则可能产生误收录、漏收录、适用对象不明确或关注级别误判。
...
- RSS 入口当前不在页面主视觉展示；`public/feed.xml` 仍作为标准订阅源生成并通过 XML 回归测试。
...
1. 将测试依赖和 CI 测试步骤规范化，确保 PR 自动运行测试。
2. 建立独立任务，明确开源策略并统一 README 与 `LICENSE` 的授权声明。
3. 增强采集状态；详细的逐条跳过失败审计作为 MVP 之后的改进评估。
4. 增加正文质量、发布日期、官方 URL 和专用 API 字段的来源级回归样本。
5. 设计同 URL 政策修订与失效检测。
6. 在上述基础稳定后，再评估新增来源、通知集成或可选大模型分析。
