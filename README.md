...
JavaScript 驱动来源还可配置 `collector` 和对应的公开 API 参数。当前国家金融监督管理总局与中国政府网已使用专用采集器。
...
- 部分政府网站可能限制GitHub海外IP或临时改版；总局采集器会尝试其官方 Big5 备用接口，其他失败仍会显示在来源状态中且不会阻断后续来源。
- 当前不执行浏览器 JavaScript；总局和中国政府网通过已验证的公开接口采集，仍不处理验证码、登录页面或未适配的强制 JavaScript 来源。
- PDF附件仅保留原链接，不对PDF做全文分析。
- 第一版不接大模型API，不产生调用费用。

## 后续最值得增加

1. 增加来源级接口字段、正文质量和官方 URL 回归样本；
2. 增加国家数据局、工信部、证监会；
3. 增加政策修订和失效检测；
4. 增加飞书或企业微信Webhook；
5. 可选大模型深度分析。
